...
The decision-maker inside the loop above is a \emph{LLM} (\textbf{L}arge \textbf{L}anguage \textbf{M}odel). It reads a sequence of tokens and predicts the next one.
...
See:
\begin{itemize}
  \item \url{https://api-docs.deepseek.com/quick_start/pricing/}
  \item \url{https://developers.openai.com/api/docs/pricing}
\end{itemize}
...
Run \texttt{ollama show qwen3:8b}:
...
See \url{https://api-docs.deepseek.com/guides/tool_calls}.
...
\begin{center}
\small
\begin{tabularx}{0.98\textwidth}{@{}>{\raggedright\arraybackslash}X ll@{}}
  \toprule
  \textbf{Query} & \textbf{Tools expected, in order} & \textbf{Answer must contain} \\
  \midrule
  What is the weather in Tokyo?     & \texttt{get\_weather} & \texttt{Tokyo}, \texttt{18} \\
  What is the weather in Reykjavik? & \texttt{get\_weather} & \texttt{Reykjavik}, \texttt{2} \\
  What is 42 + 38?                  & \texttt{add}          & \texttt{80} \\
  Hello! What can you do?           & \emph{(none)}         & \texttt{weather}, \texttt{add} \\
  \bottomrule
\end{tabularx}
\end{center}

\noindent What the fields demand:
\begin{itemize}
  \item \textbf{\texttt{expect\_tools}} --- routing: the model must pick the
    right tool \emph{because the schema advertised it}, and call tools in a
    working order.
  \item \textbf{\texttt{answer\_contains}} --- grounding: the observation must
    actually reach the user, not just sit in the transcript.
  \item The last scenario is a \emph{negative} test: smalltalk must not burn a
    tool call.
\end{itemize}
...
\begin{verbatim}
请你写一个基本的 ReAct agent，带有 weather tool
\end{verbatim}
